...
{
    public class EnvironmentMapperChunk : MonoBehaviour
    {
        public static EnvironmentMapperChunk Instance { get; private set; }

        [SerializeField] private ComputeShader shader = null;
        [SerializeField] public float metersPerVoxel = 0.1f;
        [SerializeField] private float dispatchesPerSecond = 5f;

        public RenderTexture dynamicVolume; // We will create this in code
        ...
        private void Start()
        {
            ...
            int vWidth = 128; // Let's use a smaller test size for now
            int vHeight = 128;
            int vDepth = 128;

            dynamicVolume = new RenderTexture(vWidth, vHeight, 0, RenderTextureFormat.RHalf);
            dynamicVolume.dimension = UnityEngine.Rendering.TextureDimension.Tex3D;
            dynamicVolume.volumeDepth = vDepth;
            dynamicVolume.enableRandomWrite = true; // CRITICAL
            dynamicVolume.Create(); // CRITICAL

            // Now, update your kernels to use this new texture
            clearKernel = new(shader, "Clear");
            clearKernel.Set("volume", dynamicVolume); // Use the new texture

            integrateKernel = new(shader, "Integrate");
            integrateKernel.Set("volume", dynamicVolume); // Use the new texture
            shader.SetInts("volumeSize", vWidth, vHeight, vDepth);


            ClearVolume(dynamicVolume);
        ...
        //}
        public void ClearVolume(RenderTexture volumeToClear)
        {
            // You still need to tell the kernel WHICH texture to operate on
            clearKernel.Set("volume", volumeToClear);

            // Now, just call your super-clean helper method!
            clearKernel.DispatchGroups(volumeToClear);

            Debug.Log($"Cleared volume {volumeToClear.name}");
        }
