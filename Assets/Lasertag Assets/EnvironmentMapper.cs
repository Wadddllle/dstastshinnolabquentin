...
namespace Anaglyph.XRTemplate
{
	public class EnvironmentMapper : MonoBehaviour
	{
		...
		[SerializeField] public RenderTexture volume;
        //[SerializeField] public RenderTexture weights; //here
        ...
        //private int weightsID = Shader.PropertyToID("agWeights"); //here
		...
		{
			/*
            if (weights == null)//here
            {
                weights = new RenderTexture(vWidth, vHeight, 0, RenderTextureFormat.R8, RenderTextureReadWrite.Linear)
                {
            ...
            //clearKernel.Set(nameof(weights), weights); // Pass weights to the clear kernel//here

            integrateKernel = new(shader, "Integrate");
			integrateKernel.Set(nameof(volume), volume);
            //integrateKernel.Set(nameof(weights), weights); // Pass weights to the integrate kernel//here
		...
		public void ApplyScan(Texture depthTex, Texture normTex, Matrix4x4 view, Matrix4x4 proj)//, bool useDepthFrame)
		{
            ...
            //shader.SetTexture(integrateKernel.index, weightsID, weights);//here
