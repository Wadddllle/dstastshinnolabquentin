...
public class InstructorPlacementTool_Hostage : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // --- 2. RAYCAST LOGIC ---
        bool hitSomething = Physics.Raycast(ray, out hit, rayDistance, floorLayer | hostageLayer);
    ...
    void PlaceNewObject(Vector3 hitPoint)
    {
        ...
        newObj.name = $"Hostage_{System.DateTime.Now.Ticks % 10000}"; // e.g., Target_4921

        // 4. REGISTER WITH DATABASE
        if (SessionManager.Instance != null)
        {
            SessionManager.Instance.RegisterHostage(newObj);
            Debug.Log($"[Instructor] Hostage Placed & Registered: {newObj.name}");
        }
        else
        {
        ...
        HostageAI hostageAI = newObj.GetComponent<HostageAI>();
        if (hostageAI != null && hostageConfig != null)
        {
            hostageAI.ApplyConfig(hostageConfig);

        }

    }

    void HandleHostageHover(GameObject hostage)
    {
        // If we are already highlighting this specific enemy, stop.
        if (_hoveredObject == hostage) return;

        // If we were highlighting a DIFFERENT enemy, reset that one first.
        ClearHover();

        _hoveredObject = hostage;
